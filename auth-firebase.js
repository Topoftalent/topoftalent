...
function fanNum() { var ts = Date.now().toString().slice(-6); var rnd = Math.floor(Math.random()*100).toString().padStart(2,'0'); return '#'+ts+rnd; }
...
async function getUserDoc(uid) {
  try { var snap = await getDoc(doc(db, 'users', uid)); return snap.exists() ? snap.data() : null; }
  catch(e) { return null; }
}

async function saveUserDoc(uid, data) {
  try { await setDoc(doc(db, 'users', uid), data, { merge: true }); }
  catch(e) { console.error('saveUserDoc:', e); }
}
...
.tot-modal{
  ...
  box-shadow:0 0 0 1px rgba(200,108,255,.08),0 40px 100px rgba(0,0,0,.85),0 0 80px rgba(200,108,255,.06);
}
...
.tot-modal-beam{position:absolute;top:0;left:0;right:0;height:1px;z-index:10;pointer-events:none;overflow:hidden;border-radius:20px 20px 0 0;}
.tot-modal-beam::after{content:'';position:absolute;top:0;left:-60%;width:50%;height:100%;background:linear-gradient(to right,transparent,rgba(200,108,255,.8),transparent);animation:mbeam 3.5s ease-in-out infinite;}
@keyframes mbeam{0%{left:-60%}100%{left:120%}}
.tot-modal > *{position:relative;z-index:1}
.tot-modal > .tot-modal-beam{z-index:2}

/* MODAL HEADER */
.tot-mh{padding:26px 28px 18px;border-bottom:1px solid rgba(255,255,255,.06);display:flex;align-items:flex-start;justify-content:space-between;}
...
.tot-input{width:100%;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:10px;color:#fff;font-family:'JetBrains Mono',monospace;font-size:12px;padding:11px 14px;margin-bottom:13px;outline:none;transition:border-color .2s,background .2s,box-shadow .2s;}
...
.tot-select{width:100%;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:10px;color:#fff;font-family:'JetBrains Mono',monospace;font-size:12px;padding:11px 14px;margin-bottom:13px;outline:none;transition:border-color .2s;appearance:none;}
...
.tot-btn{width:100%;background:#fff;color:#000;font-weight:700;font-size:11px;letter-spacing:.18em;text-transform:uppercase;border:none;padding:13px;margin-top:8px;border-radius:10px;display:flex;align-items:center;justify-content:center;gap:10px;transition:all .22s;position:relative;overflow:hidden;}
...
.tot-spinner{display:inline-block;width:16px;height:16px;border:2px solid rgba(255,255,255,.2);border-top-color:#fff;border-radius:50%;animation:spin .7s linear infinite;flex-shrink:0}
    ...
    <div class="tot-section active" id="reg-step1">
      <div class="tot-providers">
        <div class="tot-provider" onclick="TotAuth.socialReg('Google')">
          <div class="tot-provider-icon">G</div>Continuar con Google
        </div>
      </div>
...
onAuthStateChanged(auth, async function(fbUser) {
  if (fbUser) {
    var userDoc = await getUserDoc(fbUser.uid);
    window._totCurrentUser = userDoc || { uid: fbUser.uid, email: fbUser.email, username: fbUser.email.split('@')[0] };
  } else {
    window._totCurrentUser = null;
  }
  TotAuth._updateBtn();
...
window.TotAuth = {
  ...
  socialReg: function(provider) {
    if (provider !== 'Google') { TotAuth.showToast('Próximamente'); return; }
    signInWithPopup(auth, googleProvider).then(async function(result) {
      var fbUser = result.user;
      var existing = await getUserDoc(fbUser.uid);
      if (existing) {
        // Already has account → login flow
        window._totCurrentUser = existing;
        TotAuth.closeRegister();
        TotAuth._updateBtn();
        TotAuth.showToast('Hola, @' + existing.username + '!');
        return;
      }
      // New Google user → needs username + DOB
      _regProvider = 'Google';
      _pendingUser = { uid: fbUser.uid, provider: 'google', email: fbUser.email, fanNum: fanNum(), createdAt: today() };
      document.getElementById('reg-social-name').textContent = 'Google';
      TotAuth._showRegStep(2);
      document.getElementById('reg-step2b').classList.add('active');
    }).catch(function(e) {
      console.error(e);
      TotAuth.showToast('Error al conectar con Google');
    });
  },
  ...
  submitEmailReg: function() {
    ...
    var ok = true;
    if (!isValidEmail(email))  { document.getElementById('reg-email-err').classList.add('show'); ok=false; } else document.getElementById('reg-email-err').classList.remove('show');
    if (!isValidUsername(user)){ document.getElementById('reg-user-err').classList.add('show');  ok=false; } else document.getElementById('reg-user-err').classList.remove('show');
    ...
    _pendingUser = { provider:'email', firstName:first, lastName:last, email:email, username:user, country:country, dob:dob, age:calcAge(dob), fanNum:fanNum(), createdAt:today() };
    TotAuth.goTC();
  },

  goTC: function() {
    if (_regProvider === 'Google') {
      ...
      if (!user || !country || !dob) { TotAuth.showToast('Completa todos los campos'); return; }
      if (!isValidUsername(user)) { document.getElementById('reg-s2b-user-err').classList.add('show'); return; }
      ...
      _pendingUser.dob = dob; _pendingUser.age = calcAge(dob);
    }
  ...
  completeRegistration: async function() {
    ...
    btn.innerHTML = '<span class="tot-spinner"></span> Creando cuenta…';
    btn.disabled = true;
    try {
      var fbUser;
      if (_pendingUser.provider === 'email') {
        var cred = await createUserWithEmailAndPassword(auth, _pendingUser.email, document.getElementById('reg-pwd').value);
        fbUser = cred.user;
        await updateProfile(fbUser, { displayName: _pendingUser.firstName + ' ' + _pendingUser.lastName });
      } else {
        // Google: user already signed in via popup, just save doc
      ...
      document.getElementById('out-date-val').textContent = userData.createdAt;
      TotAuth._showRegStep(4);
      document.getElementById('reg-step4').classList.add('active');
      TotAuth._updateBtn();
      setTimeout(function(){ TotAuth.showToast('Hola, @' + userData.username + '!'); }, 400);
    } catch(e) {
      console.error(e);
      var msg = e.code === 'auth/email-already-in-use' ? 'Ese email ya está registrado' : 'Error al crear cuenta';
      document.getElementById('reg-email-err').textContent = msg;
  ...
  socialLogin: function(provider) {
    ...
    signInWithPopup(auth, googleProvider).then(async function(result) {
      ...
      if (existing) {
        ...
        // No profile doc → send to register to complete profile
        TotAuth.closeLogin();
    ...
    }).catch(function(e) { console.error(e); TotAuth.showToast('Error al conectar con Google'); });
  },
  ...
  submitLogin: async function() {
    ...
    var btn    = document.querySelector('#tot-log-backdrop .tot-btn');
    if (btn) { btn.innerHTML = '<span class="tot-spinner"></span> Entrando…'; btn.disabled = true; }
    try {
  ...
  // ── PROFILE ──
  openProfile: async function(tab) {
    document.getElementById('tot-dropdown').classList.remove('open');
  ...
  saveProfile: async function() {
    var u = window._totCurrentUser; if (!u) return;
    var newUsername = document.getElementById('prof-user').value.trim();
    if (!isValidUsername(newUsername)) { TotAuth.showToast('Usuario inválido'); return; }
  ...
  showDeleteInput:   function() { document.getElementById('delete-sad').style.display='none'; document.getElementById('delete-confirm').style.display='block'; document.getElementById('delete-confirm-input').value=''; },
  cancelDelete:      function() { document.getElementById('delete-sad').style.display='none'; document.getElementById('delete-confirm').style.display='none'; },

  deleteAccount: async function() {
    var val = document.getElementById('delete-confirm-input').value.trim();
    if (val !== 'ELIMINAR') { TotAuth.showToast('Escribe ELIMINAR para confirmar'); return; }
    var u = window._totCurrentUser; if (!u) return;
    try {
      await deleteDoc(doc(db, 'users', u.uid));
      await auth.currentUser.delete();
      window._totCurrentUser = null;
      TotAuth.closeProfile(); TotAuth._updateBtn();
      TotAuth.showToast('Cuenta eliminada');
    } catch(e) {
      // Firebase may require recent login for delete
      TotAuth.showToast('Vuelve a iniciar sesión para eliminar');
    }
  },
  ...
  checkUsername: function(input) {
    ...
    if (v && !isValidUsername(v)) { err.classList.add('show'); input.classList.add('error'); }
